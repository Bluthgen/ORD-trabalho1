...
void gravaInvertida(){
    ...
    for(int i= 0; i<= num; i++){
        ...
        fwrite(&id, sizeof(int), 1, ind);
        fwrite(&raca, sizeof(int), 1, ind);
        fwrite(&offset, sizeof(long), 1, ind);
        fwrite(&prox, sizeof(long), 1, ind);
    }
}
...
void gravaSecundaria(){
    ...
    for(int i= 0; i<= num; i++){
        ...
        fwrite(&id, sizeof(int), 1, ind);
        fwrite(&indice, sizeof(long), 1, ind);
    }
}
...
//dialogo ta estranho, vou verificar amanha
/*void dialogo(){
...
int main(){
    ...
    */
    dialogo();
    grava();
    /*printf("Vai ler um novo\n");
	povoaArquivo("individuos2.txt");
    printf("Lido 2\n");
	monta_lista();
    printf("Montado 2\n");
    buscaPorId(3);
